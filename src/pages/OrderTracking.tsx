...
const OrderTracking = () => {
  ...
  useEffect(() => {
    ...
    switch (order.status) {
      ...
      case "preparing":
        setEta(
          `${Math.max(25 - elapsedMins, 5)}-${Math.max(35 - elapsedMins, 15)} min`,
        );
        break;
      case "on_the_way":
        setEta(
          `${Math.max(10 - elapsedMins, 2)}-${Math.max(15 - elapsedMins, 5)} min`,
        );
        break;
      case "delivered":
        setEta("Delivered");
                ...
                return (
                  <div key={step} className="flex items-start gap-4">
                    <div className="flex flex-col items-center">
                      ...
                      >
                        <StepIcon
                          className={`w-5 h-5 ${isCompleted ? "text-primary-foreground" : "text-muted-foreground"}`}
                        />
                      </motion.div>
                      {i < statusSteps.length - 1 && (
                        <div
                          className={`w-0.5 h-8 ${isCompleted ? "bg-primary" : "bg-muted"}`}
                        />
                      )}
                    </div>
                    <div className="pt-2">
